 ...
 */
const findNodeByText = (container, text, exact) => Array.from(container.querySelectorAll('*'))
...
const findAndRemoveIncludingText = (container, text) => {
  const node = findNodeByText(container, text, false);
  if (node) {
...
const findIncludingTextAndInsertHTML = (container, text, html) => {
  const node = findNodeByText(container, text, false);
  if (node) {
 ...
 */
const findListByText = (container, text, count, exact = true) => {
  const node = findNodeByText(container, text, exact);
  if (!node) {
